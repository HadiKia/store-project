...
function Card({ data }) {
  ...
  return (
    <Link to={`/products/${id}`} key={id} className="w-full flex flex-col items-start justify-between text-[#000C22] px-2 py-3 md:px-4 md:py-5 lg:px-5 xl:px-6 border-y border-r overflow-hidden">
        <div className="w-full grid place-items-center p-3 md:p-4">
          <img src={image} alt={title} className="w-full h-40 md:h-56 lg:h-52 xl:h-64 xl:w-[200px] mb-5"/>
        </div>
